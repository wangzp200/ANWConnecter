...
{
    internal class SyncInventoryReceipts : ServiceJob
    {
                ...
                try
                {
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        if (oStockTransfer.GetByKey(docEntry))
                        {
                            ...
                            var toTransReason = row["U_TransReason"].ToString(); //门店补货
                            var oLines = oStockTransfer.Lines;
                            if (toTransReason != "门店补货")
                            {
                                continue;
                            } //门店补货
                            ...
                            var httpResult =
                                AnwHelper.GetHttpResult("https://app1.sapanywhere.cn/sbo/InventoryReceipts/createBO",
                                    null);
                            ...
                            var inventoryReceipts = jObject;
                            var warehouse = (JObject)inventoryReceipts["warehouse"];
                            ...
                            var parameters = new Dictionary<string, string>
                            {
                                ...
                                {"$filter", "(status+eq+'Active')+and+(whsName+eq+'" + toWhsName + "')"},
                                {"$orderby", "whsCode+asc"},
                                {"$top", "50"},
                                {"$skip", "0"}
                            };
                            ...
                            var lines = (JArray)inventoryReceipts["lines"];
                            var rowNum = 1;
                            ...
                            for (var i = 0; i < oLines.Count; i++)
                            {
                                ...
                                if (oLines.FromWarehouseCode == foWhsCode && oLines.WarehouseCode == toWhsCode)
                                {
                                    ...
                                    var line = new JObject
                                    {
                                        ...
                                        new JProperty("receiptQuantity", oLines.Quantity.ToString()),
                                        new JProperty("rowNum", rowNum),
                                        new JProperty("unitPrice", oLines.Price.ToString()),
                                        new JProperty("receiptType", toTransReason)
                                    };
                                    //入库
                            ...
                            inventoryReceipts["issueTotal"] = receiptTotal.ToString();
                            inventoryReceipts["receiptType"] = toTransReason;
                            inventoryReceipts["receiptTotal"] = receiptTotal.ToString();
                            httpResult =
                                AnwHelper.AddObjectHttpResult("https://app1.sapanywhere.cn/sbo/InventoryReceipts",
                                    inventoryReceipts, xBusinessToken);
                            if (httpResult.StatusCode == HttpStatusCode.Created)
                            {
                                ...
                                if (result != 0)
                                {
                                    ...
                                    LogHelper.WriteLog("添加 InventoryReceipts 失败",
                                        new Exception("InventoryReceipts:" + docNum + "，错误信息:" + errMsg + "-" + errCode));
                                }
                                else
                                {
                                    LogHelper.WriteLog("添加 InventoryReceipts 成功:" + docNum);
                                }
                            }
                        }
                    }
