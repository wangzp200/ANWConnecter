...
{
    internal class SyncInventoryGenExit : ServiceJob
    {
            ...
            while (!CancelTokenSource.IsCancellationRequested)
            {
                try
                {
                    ...
                    parameters.Add("$filter",
                        "((status+eq+'Close')+and+(issueType+eq+'门店领用')+and+((ext_default_UDF26+eq+null)+or+(ext_default_UDF26+eq+'')))");
                    ...
                    var inventoryGenExitInfos = jObject["value"];
                    foreach (var inventoryGenExitInfo in inventoryGenExitInfos)
                    {
                        var docNum = inventoryGenExitInfo["docNum"].ToString();
                     
                        var id = inventoryGenExitInfo["id"].ToString();
                        var issueType = inventoryGenExitInfo["issueType"].ToString(); //门店领用
                        httpResult =
                        ...
                        else
                        {
                            ...
                            inventoryGenExit["ext_default_UDF26"] = string.Format("{0:u}", DateTime.Now);
